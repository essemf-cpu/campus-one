import"../../../modulepreload-polyfill-Dezn_h7o.js";import{_ as e,h as t,l as n,m as r,y as i}from"../../../authService-DX3gg-GL.js";import{t as a}from"../../../authGuard-ykA3SEVG.js";import{t as o}from"../../../referentielService-BQbj88SN.js";import{t as s}from"../../../sidebar-n5m1VV9L.js";a(`agent`,async({profile:a})=>{if(a.service!==`Service de l'Hébergement`)return;await s(a);let c=document.getElementById(`page-title`);c&&(c.textContent=a.affectation||``);let l=document.getElementById(`historique-body`);if(!l){console.error(`❌ historique-body introuvable`);return}let u=document.getElementById(`historique-search`),d=document.getElementById(`historique-sort`),f=document.getElementById(`historique-type`),p=document.getElementById(`historique-statut`),m=document.getElementById(`historique-periode`),h=document.getElementById(`historique-annee-container`),g=document.getElementById(`historique-annee`),_=document.getElementById(`historique-mois-container`),v=document.getElementById(`historique-mois`),y=[],b=``,x=`recent`,S=`tous`,C=`tous`,w=`toutes`,T=``,E=``,D=[];try{D=await o()}catch(e){console.error(`❌ Erreur récupération types de travaux :`,e)}let O=new Map;D.forEach(e=>{O.set(e.id,e.nom)}),f&&(f.innerHTML=`
            ...
            `,document.body.classList.add(`loaded`);return}let B=r(t(i(n,`demandes_etudiants`),e(`site`,`==`,R),e(`pavillon`,`==`,z)),e=>{console.log(`📡 Historique mis à jour en temps réel :`,e.size),y=[],e.forEach(e=>{let t=e.data();t.statut!==`termine`&&t.statut!==`forclos`&&t.statut!==`non_termine`||y.push({id:e.id,...t})}),console.log(`📚 Demandes historiques :`,y.length),I(),L(),document.body.classList.add(`loaded`)},e=>{console.error(`❌ Erreur historique demandes :`,e),l.innerHTML=`
