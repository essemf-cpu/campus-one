import"../../../modulepreload-polyfill-Dezn_h7o.js";import{_ as e,b as t,g as n,h as r,l as i,m as a,y as o}from"../../../authService-DX3gg-GL.js";import{t as s}from"../../../sessionManager-DuiOh9M5.js";import{t as c}from"../../../authGuard-ykA3SEVG.js";import{t as l}from"../../../referentielService-BQbj88SN.js";c(`etudiant`,async({profile:c})=>{let u=document.getElementById(`demandes-container`);if(!u){console.error(`❌ demandes-container introuvable`);return}let d=c.matricule;if(!d){u.innerHTML=`
            ...
            `,T.forEach(e=>{v.innerHTML+=`
                    ...
                    `;return}u.innerHTML=``,e.forEach(e=>{let t=e.statut||`en_attente`,n=`En attente`,r=`status-attente`;t===`en_cours`?(n=`En cours`,r=`status-cours`):t===`termine`?(n=`Terminée`,r=`status-termine`):t===`forclos`?(n=`Forclos`,r=`status-forclos`):t===`non_termine`&&(n=`Non terminée`,r=`status-non-termine`);let i=E(e),a=i.getTime()===0?`-`:i.toLocaleDateString(`fr-FR`),o=``;e.cause&&(o=`
                        ...
                        `})};g&&g.addEventListener(`input`,()=>{x=g.value,A()}),_&&_.addEventListener(`change`,()=>{S=_.value,A()}),v&&v.addEventListener(`change`,()=>{C=v.value,A()}),y&&y.addEventListener(`change`,()=>{w=y.value,A()});let j;j=r(o(i,`demandes_etudiants`),e(`matricule`,`==`,d)),a(j,e=>{console.log(`📋 Toutes les demandes du matricule :`,e.size),b=[],e.forEach(e=>{let t=e.data();t.anneeAcademique===p&&b.push({id:e.id,...t})}),console.log(`📋 Demandes pour`,p,`:`,b.length),A()},e=>{console.error(`❌ Erreur écoute demandes :`,e),u.innerHTML=`
                ...
                `;try{let e=b.find(e=>e.id===l);if(!e)throw Error(`DEMANDE_INTRouvable`);if(!k(e))throw Error(`FEEDBACK_EXPIRE`);await n(t(i,`demandes_etudiants`,l),{evaluation:u,commentaire:d,feedbackAutorise:!1}),console.log(`✅ Feedback enregistré`)}catch(e){console.error(`❌ Erreur feedback :`,e),o.disabled=!1,o.innerHTML=`Valider`,e.message===`FEEDBACK_EXPIRE`?alert(`Le délai de 7 jours pour évaluer cette demande est dépassé.`):alert(`Impossible d'enregistrer votre évaluation.`)}}})});