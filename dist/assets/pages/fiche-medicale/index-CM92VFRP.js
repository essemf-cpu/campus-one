import"../../modulepreload-polyfill-Dezn_h7o.js";import{t as e}from"../../authGuard-ykA3SEVG.js";e(`etudiant`,async({profile:e})=>{let t=document.getElementById(`medical-page`);if(!t)return;t.innerHTML=`
        ...
        `;let n=document.querySelectorAll(`input[name="traitement"]`),r=document.getElementById(`traitement-details-group`);n.forEach(e=>{e.addEventListener(`change`,()=>{r.classList.toggle(`visible`,e.value===`oui`&&e.checked)})});function i(e){let t=document.querySelectorAll(`input[name="${e}"]`);t.forEach(e=>{e.addEventListener(`change`,()=>{if(e.value===`aucune`||e.value===`aucun`)e.checked&&t.forEach(t=>{t!==e&&(t.checked=!1)});else if(e.checked){let e=[...t].find(e=>e.value===`aucune`||e.value===`aucun`);e&&(e.checked=!1)}})})}i(`allergies`),i(`antecedents`),document.getElementById(`medical-form`).addEventListener(`submit`,e=>{if(e.preventDefault(),!document.getElementById(`consentement`).checked){alert(`Veuillez accepter l'enregistrement de votre fiche médicale.`);return}let t=document.getElementById(`medical-status`);t&&(t.textContent=`Fiche prête à être enregistrée`),alert(`La fiche médicale est prête. La connexion à la base de données sera ajoutée à l'étape suivante.`)}),document.body.classList.add(`loaded`)});